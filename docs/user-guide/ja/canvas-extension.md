...
| **✎** | Architecture の配置編集を開始・終了します |
| **⛶** | 操作が同期する外部の投影用ウィンドウを開きます |
| **Presenter アイコン** | 現在のスライド、次のスライド、ノートをまとめた発表者ビューを開きます |
| **16:9** | 固定 PDF/PowerPoint レイアウトをプレビューして、クリッピング警告を表示します |
| **プリンター** | デッキ全体を 16:9 PDF として書き出します |
| **P** | 編集可能な要素を残したハイブリッド 16:9 PowerPoint として書き出します |
| **📂** | ワークスペースから Markdown ファイルを読み込みます |
| **Refresh** | 元ファイルに連動させるか、読み込んだ時点のまま固定するかを切り替えます |
| **☰** | スライド一覧を開きます |
...
**⛶** を選ぶと、移動やサイズ変更ができる 1280x720 の投影用ウィンドウが開きます。
プレゼンテーション用のディスプレイへ移し、`F11` で全画面にします。
...
書き出す前に **16:9** で確認します。PDF と同じ固定 1280x720 レイアウトで表示し、
内容がはみ出す場合は警告します。手順の詳細は
