...
| `"ms-modern"` / `"新しい MS テーマ"` / `"社内テンプレ"` / `"背表紙付き"` | `ms-modern` |
...
- 各デッキは**先頭スライドを表紙（`layout: title`）／最終スライドをクロージング（`layout: closing`）**として扱います（中央寄せ＋専用背景色）。`ms-modern` テーマのときは、さらに**背表紙（`layout: backcover`）が拡張機能によって末尾に自動追加**されます。
