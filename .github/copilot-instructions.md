...
- スライドは **presentation canvas 拡張機能**（`.github/extensions/presentation/`）がネイティブ canvas にレンダリングします。プレゼン開始時に全スライドを生成して `load_deck` アクションで一括登録し、ページ送りは `goto_slide` アクションにインデックスを渡すだけ（Markdown の作り直し不要）で高速に進みます。
