...
- 各デッキは**先頭スライドを表紙（`layout: title`）**として扱います（中央寄せ＋専用背景）。途中の章見出しだけのページには **`layout: section`** を付けます（左寄せ＋テーマ色の専用背景）。デッキの締めは、**テーマに関わらず背表紙（`layout: backcover`）が拡張機能によって末尾に自動追加**されます。結び／お礼／Q&A などは通常スライドとして書きます（`layout: closing` は廃止）。
