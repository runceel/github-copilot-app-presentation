...
canvas iframe（renderer/）
  ...
  │ ⛶ ボタンで同期された外部ウィンドウを起動（必要なら F11 等で全画面化）
  ▼
...
`backcover` の各レイアウトは専用の配置を維持します。

本文もタイトルに続けて**上寄せ**で始まります（既定）。本文が短いスライドなどで、
**タイトルと本文をひとかたまりとして上下中央**に置きたいときだけ `layout: center` を
指定します。フッターは中央寄せの基準がずれないよう、下端へ固定されます。
