...
| `layout` | `title` を指定すると中央寄せの表紙レイアウト、`closing` を指定すると中央寄せのクロージング（結び）レイアウト、`backcover` を指定すると背表紙レイアウト（Microsoft ブルーのベタ塗り＋ロゴ＋著作権表示）になる。表紙・クロージング・背表紙はそれぞれ専用の背景が付き、見分けられる。通常スライドは省略 |
...
| `logo` | （`layout: backcover` のみ）背表紙左上のワードマーク文字列。省略時は `Microsoft` |
| `copyright` | （`layout: backcover` のみ）背表紙左下の著作権表示。省略時は `© Copyright Microsoft Corporation. All rights reserved.`、空にすると非表示 |
...
| `ms-modern` | 社内 PowerPoint テンプレート（`2024-07-29-theme.thmx`）風。白基調＋見出しは Microsoft ブルー、表紙は右上から食い込む Fluent のファセット（多面体）背景。**デッキ末尾に背表紙が自動で 1 枚追加される** |

> `ms-modern` は「最後に背表紙（Closing logo slide）が来る」ことをテーマの特徴として強制します。最終スライドが `layout: backcover` でなければ、拡張機能が既定の背表紙を自動で追加します（すでにある場合は追加しません）。agent 側で背表紙を書く必要はありません。文言を変えたいときだけ、末尾に `layout: backcover` のスライドを自分で用意して `logo` / `copyright` を指定します。

### テーマの選び方（ユーザーのテイストから判定）

プレゼンを依頼されたとき、ユーザーの言葉に**テーマに関わるテイスト**が含まれていれば、それに合うテーマを選んで `load_deck` の `theme` に渡します。判断の目安:

- **ms-modern** … 「ms-modern」「新しい MS テーマ」「社内テンプレ（で）」「背表紙付きで」など、この新テーマを名指しする表現。
...
- **スライド的なページ**（見出し・箇条書き・番号付きリスト・コードブロック・表・Mermaid 図・画像が主体）は、その本文を**ほぼそのまま使い**、先頭に `deck` / `kicker` / `page` / `total`（必要なら `layout: title` / `layout: closing`）のフロントマターを付けるだけです。
- **自然言語の文章ページ**（段落主体のプローズ）は、そのまま貼らずに **AI が見出し＋箇条書きのスライド形に要約・整形**してから配列に入れます。詳しくは次節を参照。

表紙や結びなど見出しだけのスライドは `layout: title` / `layout: closing` を付けると中央寄せになります。本文は短く、箇条書き中心にすると読みやすく、生成も速くなります。
...
- デッキの**最終スライドが結び／お礼／Q&A／連絡先などのクロージングとして機能**するなら `layout: closing` を付ける。
- 表紙(`title`)とクロージング(`closing`)は中央寄せになり、**それぞれ別の背景色**が付くので見分けられる。
- 元 Markdown のスライドに**明示の `layout` がある場合は上書きしない**。
- スライドが**1 枚だけ**のときは `layout: title` のみ（`closing` は付けない）。
- テーマが `ms-modern` のときは、さらに**背表紙（`layout: backcover`）が拡張機能によって末尾へ自動追加**される。agent が書く必要はなく、`layout: closing` の結びスライドはそのまま残る。
...
- フロントマター（`deck` / `kicker` / `page` / `total`、表紙なら `layout: title`、結びなら `layout: closing`）は通常どおり付ける。
